...
export default function AccountsSettings({ navigate }) {
  ...
  const Toggle = ({ checked, onChange }) => (
    <label style={{ position: "relative", display: "inline-flex", alignItems: "center", cursor: "pointer" }}>
      ...
      <div style={{
        width: 44, height: 24, borderRadius: 99, background: checked ? MIDNIGHT : "#cbd5e1",
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#f8f9ff", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex" }}>
        ...
        <aside style={{
          ...
          <div>
            <p style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color: "rgba(255,255,255,.4)", marginBottom: 16, paddingLeft: 12 }}>Menu</p>
            {[
              { icon: "dashboard", label: "Dashboard" },
              { icon: "monitoring", label: "Credit Score" },
              { icon: "account_balance_wallet", label: "Loans" },
              { icon: "payments", label: "Payments" },
            ].map((item) => (
              <a key={item.label} href="#"
                onClick={(e) => { e.preventDefault(); if (item.label === "Dashboard") navigate?.("dashboard"); }}
                style={{
            ...
            <a href="#" style={{
              display: "flex", alignItems: "center", gap: 12,
              padding: "10px 12px", borderRadius: 8, textDecoration: "none",
              background: GOLD, color: MIDNIGHT, fontSize: 14, fontWeight: 700,
            }}>
          ...
          <div style={{ background: "#fff", borderRadius: 12, padding: 16 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: MIDNIGHT, margin: "0 0 4px" }}>Need Help?</p>
            <p style={{ fontSize: 11, color: "#64748b", margin: "0 0 12px" }}>Our support team is available 24/7.</p>
            <button style={{
              width: "100%", padding: "8px 0", borderRadius: 8, fontSize: 12,
              fontWeight: 700, background: "#f8fafc", border: "1px solid #e2e8f0",
              color: MIDNIGHT, cursor: "pointer",
            }}>Contact Us</button>
          </div>
        </aside>

        {/* Main Content */}
        <main style={{ flex: 1, padding: "40px", maxWidth: 860 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <section style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: 28, boxShadow: "0 1px 4px rgba(0,0,0,.04)" }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end" }}>
                <button style={{
                  background: GOLD, color: MIDNIGHT, padding: "10px 24px", borderRadius: 8,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                {[
                  ...
                  <div key={item.key} style={{
                    ...
                    padding: "16px 0", borderBottom: i < arr.length - 1 ? "1px solid #f1f5f9" : "none",
                  }}>
                    <div>
                      <p style={{ fontSize: 14, fontWeight: 600, margin: "0 0 2px", color: "#0f172a" }}>{item.title}</p>
                      <p style={{ fontSize: 12, color: "#64748b", margin: 0 }}>{item.desc}</p>
            ...
            <section style={{ background: "#fff", borderRadius: 16, border: "1px solid #fee2e2", padding: 28, boxShadow: "0 1px 4px rgba(0,0,0,.04)" }}>
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                ...
                <button style={{
                  flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                  padding: 14, background: "#fff", borderRadius: 10, border: "1px solid #fecaca",
                  fontWeight: 700, fontSize: 13, color: "#dc2626", cursor: "pointer",
                }}>
                  <span className="material-symbols-outlined" style={{ fontSize: 18 }}>delete_forever</span>
                  Delete My Account
                </button>
              </div>
              <p style={{ fontSize: 11, color: "#94a3b8", textAlign: "center", fontStyle: "italic" }}>
                Processing a data deletion request can take up to 30 days as per regulatory guidelines.
              </p>
            </section>

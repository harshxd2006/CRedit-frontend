    ...
    if (currentStep === 0) return (
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ gridColumn: "span 2" }}><label style={labelStyle}>Aadhaar Number</label><input style={inputStyle} value={form.aadhar} onChange={e => set("aadhar", e.target.value)} placeholder="XXXX XXXX XXXX" /></div>
        </div>
      </div>
    );
    ...
    if (currentStep === 2) return (
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {form.loanAmount && form.tenure && (
          <div style={{ background: "rgba(17,66,93,.05)", borderRadius: 12, padding: 20 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 12px" }}>Estimated EMI</p>
            <p style={{ fontSize: 32, fontWeight: 900, color: MIDNIGHT, margin: 0 }}>
    ...
    if (currentStep === 3) return (
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {[
          ...
          <div key={doc.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: 20, background: "#f8fafc", borderRadius: 12, border: "1px solid #e2e8f0" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ width: 44, height: 44, borderRadius: 10, background: MIDNIGHT, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <button style={{ padding: "8px 20px", background: GOLD, color: MIDNIGHT, border: "none", borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>Upload</button>
          </div>
        ))}
      </div>
    );
    if (currentStep === 4) return (
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {[
          ...
          <div key={section.title} style={{ background: "#f8fafc", borderRadius: 12, padding: 20, border: "1px solid #e2e8f0" }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 16px" }}>{section.title}</p>
        ...
        <label style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: 16, background: "#fffbeb", borderRadius: 10, border: "1px solid #fef3c7", cursor: "pointer" }}>
          ...
          <main style={{ flex: 1, padding: "40px 48px" }}>
            <div style={{ maxWidth: 900, margin: "0 auto" }}>
              ...
              <div style={{ marginBottom: 32 }}>
                <h1 style={{ fontSize: 28, fontWeight: 900, color: "#0f172a", margin: "0 0 6px", letterSpacing: "-0.5px" }}>Apply for Loan</h1>
              ...
              <div style={{ display: "flex", alignItems: "center", marginBottom: 40, gap: 0 }}>
                {steps.map((step, i) => (
                  <div key={step} style={{ display: "flex", alignItems: "center", flex: i < steps.length - 1 ? 1 : "none" }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, cursor: "pointer" }} onClick={() => i <= currentStep && setCurrentStep(i)}>
                      <div style={{
                        width: 36, height: 36, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center",
                        background: i < currentStep ? "#22c55e" : i === currentStep ? MIDNIGHT : "#e2e8f0",
                        color: i <= currentStep ? "#fff" : "#94a3b8", fontWeight: 700, fontSize: 14,
                      }}>
                        {i < currentStep ? <span className="material-symbols-outlined" style={{ fontSize: 18 }}>check</span> : i + 1}
                      </div>
                      <span style={{ fontSize: 10, fontWeight: 600, color: i === currentStep ? MIDNIGHT : "#94a3b8", textAlign: "center", maxWidth: 80, lineHeight: 1.3 }}>{step}</span>
                    </div>
                    {i < steps.length - 1 && (
                      <div style={{ flex: 1, height: 2, background: i < currentStep ? "#22c55e" : "#e2e8f0", margin: "0 8px", marginBottom: 24 }} />
                    )}
                  </div>
                ))}
              </div>
              ...
              {/* Step Card */}
              <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: 32, boxShadow: "0 1px 4px rgba(0,0,0,.04)", marginBottom: 24 }}>
                <h2 style={{ fontSize: 18, fontWeight: 700, color: "#0f172a", margin: "0 0 24px", display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ width: 28, height: 28, borderRadius: "50%", background: MIDNIGHT, color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700 }}>{currentStep + 1}</span>
                  {steps[currentStep]}
                </h2>
                {renderStep()}
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <button onClick={() => setCurrentStep(s => Math.max(0, s - 1))} disabled={currentStep === 0}
                  style={{ padding: "12px 28px", borderRadius: 10, border: "1px solid #e2e8f0", background: "#fff", fontWeight: 700, fontSize: 14, cursor: currentStep === 0 ? "not-allowed" : "pointer", color: currentStep === 0 ? "#94a3b8" : "#0f172a" }}>
                  ← Previous
                </button>
                <button onClick={() => currentStep < steps.length - 1 ? setCurrentStep(s => s + 1) : null}
                  style={{ padding: "12px 28px", borderRadius: 10, border: "none", background: currentStep === steps.length - 1 ? "#22c55e" : GOLD, color: currentStep === steps.length - 1 ? "#fff" : MIDNIGHT, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>

...
const sidebarLinks = [
  ...
  { icon: "payments", label: "My Loans" },
  { icon: "trending_up", label: "Credit Building Journey", active: true },
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#f8f9ff", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex" }}>
        {/* Sidebar */}
        <aside style={{
          width: 288, background: MIDNIGHT, padding: "16px",
            ...
            {sidebarLinks.map((link) => (
              <a key={link.label} href="#"
                ...
                style={{
                  ...
                  color: link.active ? MIDNIGHT : "rgba(255,255,255,.6)",
                  fontWeight: link.active ? 700 : 500, fontSize: 14,
                  borderLeft: link.active ? `4px solid ${MIDNIGHT}` : "4px solid transparent",
                  transition: "all .15s",
                }}
                onMouseEnter={(e) => { if (!link.active) { e.currentTarget.style.color = "#fff"; e.currentTarget.style.background = "rgba(255,255,255,.08)"; } }}
                onMouseLeave={(e) => { if (!link.active) { e.currentTarget.style.color = "rgba(255,255,255,.6)"; e.currentTarget.style.background = "transparent"; } }}
              >
          ...
          <a href="#" style={{
            ...
            borderRadius: 8, textDecoration: "none", color: "rgba(255,255,255,.6)", fontSize: 14, marginBottom: 12,
          }}>
            <span className="material-symbols-outlined" style={{ fontSize: 20 }}>person_settings</span>
            Profile and Settings
          </a>
          <div style={{ background: "rgba(255,215,0,.1)", border: "1px solid rgba(255,215,0,.2)", borderRadius: 10, padding: 16 }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: GOLD, textTransform: "uppercase", letterSpacing: "0.12em", margin: "0 0 6px" }}>Pro Tip</p>
            <p style={{ fontSize: 12, color: "rgba(255,255,255,.7)", lineHeight: 1.6, margin: 0 }}>Maintaining your salary streak for 3 more months will unlock the "Reliable Earner" badge.</p>
          </div>
        </aside>
            ...
            <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: 28, marginBottom: 24, boxShadow: "0 1px 4px rgba(0,0,0,.04)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative", padding: "0 16px" }}>
                ...
                {[
                  ...
                  <div key={stage.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, background: "#f8f9ff", padding: "0 8px", zIndex: 1, opacity: stage.sub === "Locked" ? 0.5 : 1 }}>
                    <div style={{
